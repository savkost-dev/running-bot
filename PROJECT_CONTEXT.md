...
### Дата рождения / пол — добрать недозаполненных (03.06.2026)
backfill_profile.py заполнил пол+ДР для 7 юзеров (2,4,6,7,8,9,55). Хранятся в user_profile
(gender, birthdate — статичны, заполняются разово). Запуск: python3 backfill_profile.py [--dry].
get_profile есть в garmin/coros/polar/strava (единый формат male/female + YYYY-MM-DD).
НЕ заполнены:
- Garmin "Failed to retrieve social profile" (user 13,25,37,45,47) — get_user_profile недоступен,
  возможно ограничение прав токена. Разобраться или ввести вручную.
- COROS токены протухли (user 20 Karpov_90, 27 IstominaOlga) — нужен релогин, тогда добрать + проверить sex.
- user 5,48 — Strava-only (ДР не отдаёт) / нет данных.
COROS sex=0/1: допущение 0=муж,1=жен (по Ксении). Karpov_90 (муж) подтвердит когда оживёт токен.
