...
| Время МСК | UTC   | Дни | Задача |
|-----------|-------|-----|--------|
| 5:00      | 02:00 | вт,пт | `scheduled_cache_refresh` — VO2max из трекера + пересчёт зон + финальная нормализация. Strava УБРАНА 08.08.2026 — событийная через вебхук activity.create (oauth_server._strava_activity_ingest: fetch_raw → нормализация → refresh_athlete_cache). Ночной fetch_raw трекеров здесь НЕ делается (перенесён в wakeup_poll). |
| 06:00–09:00 каждые 15 мин | — | все | `scheduled_wakeup_poll` — опросник пробуждения: для юзеров с ночным сервисом синкает сырьё, ловит закрытую ночь → снимок (morning_*) + нормализация по факту поимки |
| 7:00      | 04:00 | вт,пт | `scheduled_morning` — утренняя рассылка |
| 7:15      | 04:15 | вс | `scheduled_cache_refresh_sunday` — то же тело, что cache_refresh |
| 7:30      | 04:30 | вс | `scheduled_morning_sunday` — утренняя рассылка (вс) |
| 20:00     | 17:00 | пн,чт,сб | `scheduled_evening` — вечерняя рассылка накануне |
| каждые 30 мин | — | все | `scheduled_new_workout_check` — поимка новых анонсов + АВТОАНАЛИЗ (Шаг 1) |

**Логика разделения cache_refresh / wakeup_poll (07.06.2026):**
- Ночное сырьё (Garmin/COROS/Polar/Whoop) + нормализация s3_* собираются в `scheduled_wakeup_poll`
  по факту закрытия ночи (06:00–09:00). Если ночь не закрылась к 9 — данные неполные, работать с ними
  смысла нет, поэтому в cache_refresh их и не тянем.
- `scheduled_cache_refresh` сдвинут на 05:00 (раньше 06:45), чтобы не пересекаться с wakeup_poll
  ни по смыслу, ни по времени. Делает только Strava-нагрузку, VO2max из трекеров, зоны.
- ⚠️ PENDING (на потом): финальная `run_normalization` в cache_refresh дублирует нормализацию
  wakeup_poll для юзеров с ночным сервисом. Strava-only юзеров wakeup_poll не трогает — им
  нормализация нужна только в cache_refresh. Решение: нормализовать в cache_refresh только
  тех, у кого `not _night_services` (убирает дубль, не ломает Strava-only). Пока не сделано.

refresh последовательный (asyncio.sleep(1) на юзера). При 300-400 юзерах — распараллелить.
...
- **Strava** — агрегатор: нагрузка 48ч, CTL/ATL/TSB, прогнозы забегов → VDOT. С 08.08.2026 событийная (вебхук activity.create, подписка 363107), не поллинг. Ёмкость 10/10, пересдача на квоту — после недели событийного трафика.
