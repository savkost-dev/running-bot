...
|-----------|-------|-----|--------|
| 5:00      | 02:00 | вт,пт | `scheduled_cache_refresh` — Strava CTL/ATL/TSB + VO2max из трекера + пересчёт зон + финальная нормализация. Ночной fetch_raw трекеров здесь НЕ делается (перенесён в wakeup_poll). |
