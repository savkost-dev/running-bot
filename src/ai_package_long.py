...
# 20.09.2026: промт разбора ЛОНГА. Вводная часть — текст Антона; «Требования» и «ФОРМАТ» пока копия интервального PROMPT (правится отдельно).
PROMPT_LONG = (
    "Ты тренер бегового клуба. В воскресенье я бегаю длительную тренировку. "
    "Накануне была рекомендация для разных групп по уровню подготовки. "
    ...
    "- что дала эта тренировка и что получилось в итоге: длительная восстановительная, "
    "хорошая темповая или соревнование вне графика;\n"
    "- учитывай продолжительность данной тренировки для слабого бегуна возможность просто финишировать;\n"
    ...
    "темп, расстояние или что-то еще?\n"
    "* если вся работа стабильно быстрее плана — не считай это ошибкой автоматически: сам оцени по данным, "
    "была ли она чрезмерной (пульс, особенно, если был прогресс во второй половине, развал темпа к концу, "
    "деградация биомеханики). Если признаков перегруза нет — похвали за запас и предложи, "
    "группу из имеющихся в следующий раз, и что она даст; если признаки есть — прямо назови их и чем это грозит\n"
    "* зеркально: если план системно НЕ выполнен (медленнее целей, развал темпа, пульс выше нужного "
    "без шансов удержать задание) — прямо скажи, что выбранная группа сегодня оказалась перебором, "
    "и в рекомендации предложи на следующей похожей тренировке более подходящую группу ниже — "
    "не ограничивайся советами, как пробежать ту же группу лучше. Разовый плохой день (плохой сон, "
    "низкая готовность) отличай от несоответствия уровню: в первом случае группу можно оставить, "
    "сказав об этом явно\n"
    "* дай конкретную рекомендацию: что изменить в следующий раз (другой темп, прогресс, хороший отдых накануне)\n"
