...
_MODES_PROMPT = (
    ...
    "САМЫМ ПОСЛЕДНИМ элементом массива добавь сводку объёма: "
    "element=Объём качественной работы, "
    "dist=«ПАНО+10 сек/км и быстрее: X км (непрерывно Y) · МПК+5 сек/км и быстрее: Z км (непрерывно W)», "
    "intensity оставь пустым, note=1-2 предложения о том, что этот объём означает "
    "для тяжести тренировки. Непрерывный — самый длинный кусок работы без настоящего "
    "отдыха (лёгкая трусца прерывает, бег в рабочем темпе — нет).\n\n"
    "Данные тренировки:\n"
)
