...
def build_ai_b_prompt_reco_champion(analysis: dict, user_data: dict, zones_map: dict, recovery: dict | None, recovery_scenario_text: str = "", speed_ceilings: list | None = None) -> str:
    ...
    return (
        ...
        + "\nОБЪЁМ КАЧЕСТВЕННОЙ РАБОТЫ: при подборе группы сравни темпы каждой группы "
        "с индивидуальными порогами атлета. Суммируй непрерывную дистанцию/время в зонах "
        "около ПАНО и выше (темп от на 10-15 сек/км медленнее ПАНО и быстрее) и около МПК "
        "и выше (темп от на 5-10 сек/км медленнее МПК и быстрее) — учитывай все отрезки "
        "по темпу, включая активное восстановление и связки. Если суммарный непрерывный объём "
        "в этих зонах превышает ~20-25 минут, тренировка скорее «тяжёлая» или «очень тяжёлая», "
        "даже если в описании есть восстановительные отрезки. Если восстановительный темп "
        "настолько быстрый, что не даёт восстановления, серия превращается в непрерывную "
        "переменную работу (быстро + темпово): это меняет физиологию с интервальной на "
        "темповую/пороговую — учти это при выборе группы.\n"
