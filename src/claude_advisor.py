...
def build_ai_b_prompt_reco_champion(analysis: dict, user_data: dict, zones_map: dict, recovery: dict | None, recovery_scenario_text: str = "", speed_ceilings: list | None = None) -> str:
    ...
    return (
        ...
        "Будут данные конкретного атлета с его зонами. По данным атлета будет предварительно "
        "рассчитан объём в зонах ПАНО+ и МПК+, а также его скорость на коротких отрезках "
        "для скоростных работ. Используй данные, которые даны, сам не считай.\n"
        ...
        "Шаг 1 — быстрый отсев без разбора. Пройди по группам сверху вниз и по двум цифрам "
        "сводки (МПК+ и ПАНО+) задай каждой два вопроса: выполнимость не близка к нулю? "
        ...
        "на последних повторах. Оценивай по цифрам сводки, плавно, не порогами:\n"
        "– объём МПК+ и непрерывный ПАНО+ — лимиты ниже действуют ТОЛЬКО для непрерывной работы "
        "без отдыха или когда «отдых» идёт в пороговой зоне и быстрее. Если между отрезками "
        "есть трусца не быстрее easy длиной от половины рабочего отрезка — суммарные минуты "
        "из сводки для выполнимости НЕ используй (у медленных групп их больше просто потому, что "
        "та же дистанция бежится дольше — это не тяжесть); там выполнимость определяется тем, "
        "удержит ли атлет темп отрезка на всех повторах (см. потолок и правило прогрессии ниже).\n"
        "– объём МПК+ (непрерывная работа): считается СУММАРНОЕ время за тренировку — усталость копится; то, что "
        ...
        "– непрерывный ПАНО+: сверх ~35-40 мин каждая минута снижает, ≈ 0 к 50-60 у быстрых "
        ...
        "200 м потолок не применяй — там работают лимиты МПК+ и ПАНО+;\n"
        ...
        "– если «восстановительный» отрезок группы по сводке лежит в зоне threshold или "
        "быстрее — серия для этого атлета превращается в непрерывную переменную работу "
        "(быстро + темпово): физиология пороговая/темповая, не интервальная, и выполнимость ниже.\n\n"
        ...
        "а НЕ количеством минут в сводке — больше минут у медленной группы не значит больше стимула:\n"
        ...
        "– в непрерывной работе без отдыха дополнительно: ≈ 0, если МПК+ = 0 и ПАНО+ < ~20 мин;\n"
