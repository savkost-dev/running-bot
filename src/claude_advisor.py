...
def _build_analyze_prompt(raw_text: str, comments_text: str) -> str:
    ...
    return (
        ...
        "Не путай местами.\n\n"
        "ГРУППА ЗДОРОВЬЯ / начинающих (эмодзи ⏺️ или текст 'бег и ходьба'):\n"
        ...
        "active_recovery=false. НЕ выдумывай для неё темп и дистанции.\n\n"
        "ACTIVE_RECOVERY — относительно рабочего темпа, НЕ по абсолютному порогу:\n"
        "Восстановление активное (active_recovery=true), если оно медленнее рабочего\n"
        "отрезка МЕНЕЕ чем на 40-50%. Если медленнее на 50%+ — это полный отдых трусцой\n"
        "(active_recovery=false).\n"
        "Пример: работа 3:20/км, восстановление 5:00/км — разница 50% — полный отдых.\n\n"
