...
def _build_analyze_prompt(raw_text: str, comments_text: str) -> str:
    ...
    return (
        ...
        "90сек / 0.3км = 5:00 мин/км.\n\n"
        "ФОРМАТ ВРЕМЕНИ (критично):\n"
        "В анонсах клуба точка = разделитель минут и секунд, НЕ дробь.\n"
        "'1.32' = 1 минута 32 секунды = 92 сек. '1.10' = 70 сек. '1.22' = 82 сек.\n"
        "Целое число без точки = просто секунды: '60 секунд' = 60 сек, '16 секунд' = 16 сек.\n"
        "Темп в скобках типа '(4:45/6:10)' = уже посчитанный клубом темп\n"
        "(рабочий/восстановление мин/км) — используй его если есть.\n\n"
        "РАБОТА vs ВОССТАНОВЛЕНИЕ когда оба заданы временем:\n"
        "Формат '300 м – 60 секунд / 100 м – 30 секунд' означает:\n"
        "рабочий отрезок (300м за 60 сек) + восстановительный отрезок (100м трусцой за 30 сек).\n"
        "Первый отрезок (длиннее, быстрее) = work, второй (короче, медленнее) = recovery.\n"
        "Не путай местами.\n\n"
        "ГРУППА ЗДОРОВЬЯ / ходоков (эмодзи ⏺️ или текст 'бег и ходьба'):\n"
        "Особая группа без отрезков и темпа (чередование бег/ходьба).\n"
        "Выведи её в groups с work='бег/ходьба чередование', recovery_pace=null,\n"
        "active_recovery=false. Темп и дистанции НЕ выдумывай.\n"
        "Подходит для начинающих и людей без данных профиля.\n\n"
        "ACTIVE_RECOVERY (логика OR — достаточно ОДНОГО условия):\n"
        "active_recovery=true если ЛИБО темп восстановления быстрее 5:30/км,\n"
        "ЛИБО восстановление медленнее рабочего отрезка не более чем на ~50%.\n"
        "Если ни одно условие не выполнено — полный отдых, active_recovery=false.\n"
        "Пример: работа 3:20/км, восстановление 5:00/км — медленнее на 50%, но 5:00 быстрее 5:30 → true.\n\n"
        "ГРУППА 5 — бег по 4-й дорожке стадиона:\n"
        "Группа 5 часто бегает по 4-й дорожке и пересчитывает дистанции под неё\n"
        "(например 322м/430м/108м вместо 300/400/100м). Это НОРМА, не ошибка парсинга.\n"
        "Сохраняй их фактические дистанции как есть.\n\n"
        "ЕДИНАЯ СТРУКТУРА всех групп:\n"
        "Структура работы задаётся ОДИН раз в основном посте (например\n"
        "'10 по 300/100 + 400 + 10 по 100/100'). Все группы делают одну и ту же структуру,\n"
        "отличаются ТОЛЬКО темпом на отрезках. Иногда у групп 4-5 меньше повторений\n"
        "(10 вместо 12) — это допустимо, отметь отклонение, но структуру бери общую.\n\n"
        "НЕТ ЦЕЛЕВОГО ТЕМПА:\n"
        "Если у восстановительного отрезка нет ни заданного темпа, ни времени для\n"
        "пересчёта — active_recovery=false, recovery_pace=null. Не фантазируй\n"
        "и не оценивай на глаз.\n\n"
        ...
        "ДОП. ГРУППА со ссылкой на основную: 'Группа 3.5 работает с 3 группой' — это\n"
        "указание следовать программе группы 3, а НЕ отдельные отрезки. Запиши\n"
