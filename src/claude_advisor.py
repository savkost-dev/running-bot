...
def build_ai_b_prompt_reco_champion(analysis: dict, user_data: dict, zones_map: dict, recovery: dict | None, recovery_scenario_text: str = "", speed_ceilings: list | None = None) -> str:
    ...
    return (
        ...
        "СТИМУЛ — сколько тренировка даёт этому атлету в целевой для неё зоне. Определяется ПОПАДАНИЕМ "
        "рабочего темпа группы в целевую зону типа работы (см. СВЯЗЬ ТИПА РАБОТЫ С ЗОНАМИ ниже), "
        "а не количеством минут — больше минут у медленной группы не значит больше стимула:\n"
        "– максимален, когда темп группы лежит в целевой зоне (для интервалов 300-600 м — interval, "
