...
def build_ai_b_prompt_reco_champion(analysis: dict, user_data: dict, zones_map: dict, recovery: dict | None, recovery_scenario_text: str = "", speed_ceilings: list | None = None) -> str:
    ...
    return (
        ...
        "– короткие быстрые отрезки (только если суть тренировки — скоростная работа, "
        "отрезки ≤200 м) с ровным темпом: сравнивай темп группы с максимально повторяемым "
        "темпом атлета (дан ниже). Медленнее потолка или на его уровне — не снижает; "
        "быстрее на 3-5 сек/км — ≈ 50%; быстрее на 8-10 сек/км — ≈ 0. Для отрезков длиннее "
        "200 м потолок не применяй — там работают зоны и число повторов;\n"
