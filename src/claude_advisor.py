...
def strip_links(text: str | None) -> str:
    """Чистит текст анонса от ссылок и служебной вёрстки Telegram.

    Клубные анонсы содержат карты, регистрации и разметку, которые не нужны ни в брифе,
    ни в рекомендации: [текст](url) → текст, голые URL и строки-приглашения убираются,
    бэктики и подчёркивания снимаются. Применяется и при сохранении разбора,
    и при рендере (чтобы старые записи тоже отображались чисто).
    """
    if not text:
        return ""
    import re as _re_l
    t = str(text)
    t = _re_l.sub(r"!?\[([^\]]*)\]\([^)]*\)", r"\1", t)      # [текст](url) → текст
    t = _re_l.sub(r"(?:https?|tg)://\S+", "", t)             # голые ссылки
    t = t.replace("`", "").replace("__", "")                 # вёрстка
    out = []
    for line in t.splitlines():
        ...
        if not s:
            continue
        low = s.lower()
        if low.startswith(("регистрируемся", "регистрация")) or "зарегистрировать" in low:
            continue
        out.append(s)
    return "\n".join(out).strip()
...
def build_evening_prompt(workout: dict, fitness: dict, recovery: dict | None = None, weather_prompt: str = "") -> str:
    ...
    parts += [
        ...
        *(["ПОГОДА НА ТРЕНИРОВКУ:", weather_prompt,
        ...
        "   СОСТАВНОЙ рабочий отрезок (несколько темпов ПОДРЯД без отдыха между ними,",
        "   напр. 800м база + 200м ускорение): внутри RepeatGroupDTO идут ПОДРЯД несколько",
        "   interval-шагов — по одному на каждый темповой кусок, каждый со своей дистанцией и",
        "   своим pace.zone (у куска с набеганием targetValueOne/Two = быстрый/медленный край),",
        "   затем ОДИН recovery-шаг после всей связки. Масштабируется на любое число кусков",
        "   (2, 3, 5 темпов подряд — столько же interval-шагов перед recovery).",
...
def _build_analyze_prompt(raw_text: str, comments_text: str) -> str:
    ...
    return (
        ...
        "ТЕКСТ ПОСТА:\n"
        f"{raw_text}\n\n"
        "КОММЕНТАРИИ К ПОСТУ читай РАДИ ДВУХ ЦЕЛЕЙ, всё остальное в них игнорируй:\n"
        "1) Дополнительные группы с темпами (например 5 и 6) и поправки к ним — "
        "размер круга дорожки, фактические дистанции, число повторов. Такие группы "
        "равноправны с группами из анонса и ОБЯЗАТЕЛЬНО попадают в groups.\n"
        "2) Строки с пометкой [ИНСТРУКЦИЯ АДМИНИСТРАТОРА] — уточнение задания "
        "с ВЫСШИМ приоритетом: важнее основного текста анонса и любых других комментариев.\n"
        "Структуру и характер работы определяй ПО АНОНСУ: что считать отдыхом, какая "
        "физиология у отрезка, насколько тяжело — мнения участников об этом не учитывай. "
        "Бег в темпе около ПАНО и быстрее НЕ называй восстановлением или отдыхом: "
        "это смена темпа, темповый или пороговый отрезок; восстановление — только бег "
        "существенно медленнее ПАНО.\n"
        f"{comments_text}\n\n"
        "ЗАДАЧА 1 — Валидация:\n"
        ...
        "  (без повторов; темп связки, если задан анонсом, — в groups[].blocks)\n"
        "ОТДЫХ бывает двух видов, не путай их:\n"
        "- отдых, разделяющий повторы внутри серии → recovery_distance_m этого repeat-блока;\n"
        "- связующий отдых МЕЖДУ блоками работы → всегда отдельный easy-блок между ними,\n"
        "  а НЕ recovery_distance_m соседнего блока.\n"
        "purpose — физиология блока. СООТНОШЕНИЕ РАБОТА/ОТДЫХ определяет физиологию\n"
        "точнее, чем длина отрезка или темп — учитывай его в первую очередь:\n"
        "- Короткий отрезок (≤200м, ≤30 сек) + ДЛИННЫЙ отдых (в 1.5-2+ раза дольше\n"
        "  работы или >60 сек трусцы): КрФ восстанавливается, лактат накапливается\n"
        "  медленнее чем на средних отрезках, но к концу серии фон растёт (нарастающая\n"
        "  тяжесть, утомление ЦНС, не острое закисление). Степень восстановления\n"
        "  (полное/значительное/частичное) определи сам по соотношению работа/отдых —\n"
        "  не абсолютизируй. Характер: СКОРОСТЬ / нейромышечная мощность / техника на\n"
        "  скорости, НЕ МПК/ПАНО.\n"
        "- Длинный отрезок (300м+) + КОРОТКИЙ отдых (≤ длины работы): недовосстановление,\n"
        "  накопление лактата → МПК / гликолитическая выносливость / ПАНО.\n"
        "- Средние отрезки с умеренным отдыхом: МПК, аэробная мощность.\n"
        "НЕ привязывай короткие отрезки с длинным отдыхом к МПК/ПАНО — смотри на отдых.\n"
        "Длинный отдых = скоростная направленность. Опиши диапазон для разных темповых\n"
        "групп. Конкретную зону НЕ фиксируй — она определяется в рекомендации под\n"
        "пользователя.\n"
        ...
        "Привязка к структуре через 'block': N. easy-блок попадает в groups[].blocks\n"
        "ТОЛЬКО если анонс задаёт ему темп для этой группы: запись {block: N, recovery_pace}\n"
        "(без work-полей). Если темп связки не задан — easy-блок в groups не дублируй.\n"
        ...
        "СОСТАВНЫЕ СЕРИИ (segments) — повтор из нескольких шагов:\n"
        "Если ВНУТРИ повтора несколько рабочих кусков — с разными режимами, С отдыхом между\n"
        "ними или БЕЗ (напр. '800 база + 200 ускорение' или '800 / 200 отдых / 200'):\n"
        "- В structure[] у repeat-блока задай segments — шаги одного повтора ПО ПОРЯДКУ\n"
        "  выполнения. Рабочий шаг: {\"distance_m\": N, \"role\": \"...\"};\n"
        "  отдых между рабочими: {\"distance_m\": N, \"rest\": true}.\n"
        "  Правила порядка: первый сегмент серии — всегда рабочий; подряд могут идти\n"
        "  несколько рабочих с разными темпами, но НЕ два отдыха подряд.\n"
        "  work_distance_m блока = СУММА рабочих сегментов (rest не входит).\n"
        "  Серия ЗАКАНЧИВАЕТСЯ отдыхом: финальный отдых повтора — это recovery_distance_m\n"
        "  блока (НЕ rest-сегмент; последним rest-сегмент не ставь).\n"
        "- В groups[].blocks[] добавь segments с темпом КАЖДОГО сегмента (у каждой группы свой):\n"
        "  rest-сегмент включай в groups ТОЛЬКО если анонс задаёт ему темп (или время —\n"
        "  пересчитай в темп): {\"distance_m\": N, \"rest\": true, \"recovery_pace\": \"7:00\"};\n"
        "  разные отдыхи могут иметь разный recovery_pace. Без темпа — не включай.\n"
        "  \"segments\": [{\"distance_m\": 800, \"work_pace_start\": \"3:30\", \"work_pace_end\": \"3:30\"},\n"
        ...
        "  темп РОВНЫЙ. Диапазон от тренера (напр. '200 м — 50-45 сек') означает:\n"
        "  первый повтор 50 сек, последний 45 — то есть work_pace_start = темп ПЕРВОГО\n"
        "  повтора, work_pace_end = темп ПОСЛЕДНЕГО. Набегание — это СТУПЕНЬ (отрезок\n"
        "  бежится ровно, но быстрее базы), а не разгон внутри отрезка — если только\n"
        "  в анонсе ЯВНО не сказано иное («постепенно разгоняемся внутри отрезка»).\n"
        "- ТЕМП ГРУППЫ ИЗ ЗАДАНИЯ = ТЕМП БАЗОВОГО СЕГМЕНТА. Тренер задаёт темп базы\n"
        "  (напр. '8 км – темп 4:15') И ОТДЕЛЬНО время/темп ускорения (напр. '200 м – 50-45 сек').\n"
        "  Копируй темп группы в базовый сегмент КАК ЕСТЬ, ускорение считай из его собственных цифр.\n"
        "  ЗАПРЕЩЕНО трактовать темп группы как СРЕДНИЙ по связке и замедлять базу ради\n"
        "  того, чтобы среднее сошлось: километр со связкой закономерно выходит БЫСТРЕЕ\n"
        "  заявленного темпа — это норма, не ошибка.\n"
        "  Сегмент ускорения НИКОГДА не медленнее базового; внутри ускорения\n"
        "  start = медленный край, end = быстрый (число в end МЕНЬШЕ).\n"
        "- КРИТИЧНО: блок-уровневые work_pace_start/work_pace_end/work_distance_m у группы\n"
        "  ОБЯЗАТЕЛЬНО сохрани и заполни КАК ОБЫЧНО: work_distance_m = вся связка (сумма),\n"
        "  work_pace_start = темп базы (= заявленный темп группы), work_pace_end = темп финиша ускорения —\n"
        "  т.е. прогрессия по всей связке. Поля segments — ДОБАВКА, НЕ замена старых полей.\n"
        "- АКЦЕНТ В РАЗБОРЕ (обязательно): в purpose этого блока И в summary ЯВНО проговори,\n"
        "  что рабочий отрезок состоит из двух темпов БЕЗ отдыха между ними (дистанции и роли),\n"
        "  и если второй кусок с набеганием (прогрессия темпа) — скажи это прямым текстом.\n"
        "  Финишное ускорение повышает фактическую нагрузку отрезка относительно базового\n"
        "  темпа — учти это в intensity_level и what_to_watch.\n"
        "- Если рабочий отрезок ОДНОРОДНЫЙ (один режим на всю длину) — segments НЕ добавляй,\n"
        "  работай по-старому (work_distance_m + блок-уровневый темп). НЕ дроби без необходимости.\n\n"
        "============ ДЛЯ LONG-ТРЕНИРОВОК ============\n\n"
        ...
        '  "structure": [\n'
        '    {"block": 1, "type": "repeat", "reps": 10, "work_distance_m": 300, "recovery_distance_m": 100, "purpose": "длинный отрезок с коротким отдыхом (3:1) — недовосстановление, накопление лактата; МПК у быстрых групп, ПАНО/гликолитическая выносливость у умеренных"},\n'
        '    {"block": 2, "type": "easy", "distance_m": 400, "description": "лёгкий бег между блоками"},\n'
        '    {"block": 3, "type": "repeat", "reps": 10, "work_distance_m": 100, "recovery_distance_m": 200, "purpose": "короткий отрезок с длинным отдыхом (1:2) — КрФ восстанавливается полностью; скоростная/нейромышечная мощность, каждый отрезок свежим; НЕ МПК"}\n'
        "  ],\n"
...
def _wr_pace_sec(p) -> float | None:
    """'4:05' → 245.0; пусто/мусор → None."""
    try:
        m, s = str(p).strip().split(":")
        return int(m) * 60 + float(s)
    except Exception:
        return None


def _wr_mmss(sec: float) -> str:
    sec = int(round(sec))
    return f"{sec // 60}:{sec % 60:02d}"


def _work_rest_note(bl: dict, sb: dict | None) -> str:
    """17.09.2026: время работы, время отдыха и R:W для строки группы в промте Шага 2 —
    чтобы ИИ оценивал отдых временем, а не расстоянием, и не умножал сам.
    Пусто, если нет отдыха, нет темпа или блок составной (сегменты слитно)."""
    if not sb or bl.get("segments"):
        return ""
    wd = sb.get("work_distance_m")
    rd = sb.get("recovery_distance_m")
    ps = _wr_pace_sec(bl.get("work_pace_start"))
    pe = _wr_pace_sec(bl.get("work_pace_end")) or ps
    rp = _wr_pace_sec(bl.get("recovery_pace"))
    if not (wd and rd and ps and rp):
        return ""
    work_t = wd / 1000 * (ps + pe) / 2
    rest_t = rd / 1000 * rp
    if work_t <= 0:
        return ""
    return (f"; работа {wd} м ≈ {_wr_mmss(work_t)}, отдых {rd} м ≈ {_wr_mmss(rest_t)}, "
...
def build_ai_b_prompt(analysis: dict, user_data: dict, zones_map: dict, recovery: dict | None, recovery_scenario_text: str = "", speed_ceilings: list | None = None) -> str:
    """Развилка боевого промта рекомендации: challenger правится, champion заморожен."""
    fn = (build_ai_b_prompt_reco_challenger if RECO_PROMPT_VARIANT == "challenger"
          else build_ai_b_prompt_reco_champion)
    return fn(analysis, user_data, zones_map, recovery, recovery_scenario_text, speed_ceilings)


def build_ai_b_prompt_reco_champion(analysis: dict, user_data: dict, zones_map: dict, recovery: dict | None, recovery_scenario_text: str = "", speed_ceilings: list | None = None) -> str:
    ...
    json_schema = (
        ...
        + '"recommended_pace": "темп основной группы (например: 4:00–4:25 мин/км)", '
        + '"reason": "1-2 предложения: почему эта группа — укажи зоны и объём", '
    ...
    return (
        ...
        "Тебе будет предложена тренировка с чёткой структурой и заданным темпом работы. Будет несколько "
        "групп, отличающихся только по скорости. Иногда может быть меньше повторов.\n"
        "Будут данные конкретного атлета с его зонами и его скорость на коротких отрезках "
        "для скоростных работ. Используй данные, которые даны.\n"
        "Подбери группу для бегуна. Тебе нужно найти самую подходящую группу. Подходимость "
        "оценивается как произведение вероятности выполнить тренировку целиком × СТИМУЛ.\n"
        "Отвечай тренерски — без шаблонов, живым языком.\n\n"
        "Алгоритм оценки, строго по шагам.\n"
        "Шаг 1 — быстрый отсев без разбора. Сначала определи тип работы и его целевую зону "
        "(см. СВЯЗЬ ТИПА РАБОТЫ С ЗОНАМИ ниже). Затем пройди по группам сверху вниз, сравнивая "
        "темп группы с зонами атлета, и задай каждой два вопроса: выполнимость не близка к нулю? "
        "стимул не близок к нулю? Если хотя бы один ответ отрицательный — группа отброшена: "
        "НЕ смотри её темпы относительно зон, НЕ проверяй потолок и прогрессию, НЕ пиши "
        "по ней обоснований. Процент ей ставь спадом от ближайшей оценённой группы к нулю "
        "по мере удаления (слишком быстрые — risk/hard, слишком медленные — tooslow/easy). "
        "Отброшенная группа не может быть optimal, как бы точно ни попадала в зону.\n"
        "Шаг 2 — детальный разбор. Остаются обычно 2-3 реальные группы (иногда одна — это "
        "нормально) — только их сравнивай подробно по правилам ниже. Данные о восстановлении "
        "на обоих шагах не используй — только физиологию бегуна.\n\n"
        "ВЫПОЛНИМОСТЬ — вероятность дойти до конца в заданных темпах, не развалившись "
        "на последних повторах. Оценивай по темпам и структуре, плавно, не порогами:\n"
        "– СНАЧАЛА определи тип работы по структуре и темпам групп. НЕПРЕРЫВНАЯ работа — между "
        "рабочими отрезками нет трусцы или шага: восстановление 0 м, либо «смена темпа» на отрезок "
        "в зоне marathon и быстрее (это не отдых, а другая рабочая часть). ИНТЕРВАЛЬНАЯ работа — "
        "между отрезками трусца в зоне easy или медленнее длиной от половины рабочего отрезка, "
        "или шаг/стоя. Промежуточный случай — трусца короче половины отрезка или в зоне между easy "
        "и marathon: отдых неполный, работа ближе к непрерывной.\n"
        "– лимиты объёма ниже действуют только для непрерывной работы (и с оговоркой — для "
        "промежуточного случая). В интервальной работе суммарные минуты в зонах для выполнимости "
        "не считай — у медленных групп их больше просто потому, что та же дистанция бежится "
        "дольше; там выполнимость определяется тем, удержит ли атлет темп отрезка на всех "
        "повторах (см. потолок и правило прогрессии ниже).\n"
        "– объём в зоне МПК и быстрее (непрерывная работа): считается СУММАРНОЕ время за тренировку — усталость копится; то, что "
        "оно разбито на короткие куски, облегчает лишь немного и границ не сдвигает. "
        "До 6-8 мин не снижает, каждая минута сверх снижает, к 12-13 мин вероятность ≈ 50%, "
        "к 14-15 мин ≈ 0 (в переменной работе без отдыха возможностей меньше: снижение "
        "сверх 5-6 мин, ≈ 50% к 8 мин, ≈ 0 к 10);\n"
        "– непрерывная работа в пороговой зоне и быстрее: сверх ~35-40 мин каждая минута снижает, ≈ 0 к 50-60 у быстрых "
        "групп и к 40-50 у медленных;\n"
        "– условия независимы — достаточно одного, чтобы вероятность упала;\n"
        "– короткие быстрые отрезки с ровным темпом: сравнивай темп группы с максимально повторяемым "
        "темпом атлета (дан ниже) или, если его нет, с зоной repetition. ВАЖНО: и потолок, и зона repetition — "
        "это не темп одного повтора, а темп, который атлет качественно держит до ~10 повторов на отрезках 400-800 м "
        "и до 20-25 повторов на отрезках до 200 м с полноценным "
        "отдыхом; дальше начинается быстрое западание; при активном отдыхе (быстрее easy) — на 2-4 повтора меньше. "
        "Поэтому темп на уровне потолка/repetition или медленнее при числе повторов до 10 — выполним с высокой "
        "вероятностью, не снижай его «на глаз». Быстрее потолка на 3-5 сек/км — ≈ 50%; быстрее на 8-10 сек/км — ≈ 0;\n"
        "– число повторов и отдых сдвигают эти границы: мало повторов (≤ 6) и длинный "
        "отдых — снижение начинается позже; много повторов (≥ 10), отрезки длиннее 200 м "
        "или короткий/быстрый отдых — раньше;\n"
        "– если у группы прогрессия (диапазон темпа), правило другое: сравнивай с потолком "
        "СЕРЕДИНУ диапазона, а не финишный край. Превышение потолка на 1-2 сек/км "
        "выполнимо и на всех повторах; чем больше превышение, тем меньше повторов "
        "на нём реально удержать. Выполнимость падает, только когда быстрее потолка "
        "на 5+ сек/км идёт больше трети повторов.\n\n"
        "СТИМУЛ — сколько тренировка даёт этому атлету в целевой для неё зоне. Стимул МОНОТОННО РАСТЁТ "
        "от медленных групп к быстрым и НЕ падает у групп быстрее целевой зоны — быстрые группы "
        "ограничивает только выполнимость. Определяется попаданием рабочего темпа группы "
        "в целевую зону типа работы (см. СВЯЗЬ ТИПА РАБОТЫ С ЗОНАМИ ниже), "
        "а не количеством минут — больше минут у медленной группы не значит больше стимула:\n"
        "– высокий, когда темп группы лежит в целевой зоне или быстрее (для интервалов 300-600 м — interval, "
        "для длинных отрезков — threshold, для коротких ≤200 м — repetition);\n"
        "– падает с каждой зоной ниже целевой: группа на зону ниже — стимул слабый, на две — ≈ 0; "
        "база медленнее marathon-темпа атлета — ≈ 0, даже если пробежать её тривиально;\n"
        "– в непрерывной работе дополнительно: ≈ 0, если весь объём идёт медленнее пороговой зоны;\n"
        "– для скоростной работы (короткие отрезки) смотри на попадание в зону repetition "
        "(см. СВЯЗЬ ТИПА РАБОТЫ С ЗОНАМИ ниже): стимул тем выше, чем быстрее группа, "
        "которую атлет ещё выполнит целиком без срыва; заметно медленнее потолка — "
        "стимула почти нет.\n\n"
        "ПОДХОДИМОСТЬ: максимальные проценты — там, где оба множителя высоки. Ориентиры "
        "по шкале: 85-100 — точное попадание в целевые зоны, вероятность выполнить "
        "целиком в целевых темпах очень высокая, срыв маловероятен; "
        "65-85 — хороший вариант с небольшим компромиссом по стимулу или выполнимости, "
        "вероятность выполнить высокая, но с ощутимым риском недовыполнения "
        "на последних повторах; "
        "40-65 — рабочий запасной вариант с ощутимой потерей; 0-25 — отброшенные на шаге 1.\n\n"
        ...
        "Группа 3.5 — промежуточная: быстрее группы 4, но медленнее группы 3. "
        "Группа 1 — самая быстрая. Группы с меньшим номером всегда быстрее групп с большим номером.\n"
        "Могут встречаться группы с дробным номером (обычно 2.5 или 3.5): 2.5 быстрее 3, "
        ...
        "Правило эпитетов: у recommended_group эпитет всегда optimal, какой бы ни был её процент. "
        "Группы с номером МЕНЬШЕ рекомендованной — быстрее и тяжелее "
        ...
        "ВАЖНО: верни suitability_percentages для ВСЕХ групп из раздела ГРУППЫ выше "
        "(только тех, что там реально перечислены), без пропусков и без добавления своих. "
        "НЕ включай группу здоровья в suitability_percentages — "
        "она не оценивается процентом. "
        "Если группа слишком быстрая — ставь risk или hard с низким %, "
        "если слишком медленная — tooslow или easy.\n"
        ...
        "В if_feeling_good / if_tired предлагай переход ТОЛЬКО в группы с подходимостью ≥50%. "
        "Если такой альтернативы нет — советуй остаться в рекомендованной группе и скорректировать усилие.\n"
        ...
        "ВОССТАНОВЛЕНИЕ (TR/Recovery Score) на проценты и recommended_group НЕ влияет — они из оценки "
        "без TR. Если восстановление низкое: когда группа на ступень медленнее рекомендованной "
        "прошла отсев (стимул есть, подходимость ≥50%) — добавь в reason в скобках: "
        "«(сегодня, при низком восстановлении, разумнее группа X — меньший объём)»; "
        "когда такой группы нет — оставь рекомендованную и в if_tired/preparation_tips предложи "
        "её облегчённый вариант (мягче темп или меньше повторов/серий — на усмотрение атлета). "
        "При нормальном восстановлении ничего не добавляй. "
        "lowered_by_recovery всегда false, group_if_recovered всегда null.\n"
        "Отвечай только JSON, без лишнего текста."
    )
...
def build_ai_b_prompt_reco_challenger(analysis: dict, user_data: dict, zones_map: dict, recovery: dict | None, recovery_scenario_text: str = "", speed_ceilings: list | None = None) -> str:
    """Промпт варианта B: ИИ видит Хшаг 1 + данные бегуна и САМ выбирает группу.
    Без формул — чистая рекомендация от ИИ.
    """
    groups = analysis.get("groups") or []
    ...
    return (
        ...
        "3. ТИП РАБОТЫ\n\n"
        "ТИПЫ РАБОТЫ (СТАДИОН)\n"
        "1. Непрерывная — бег без остановок и без отдыха, темп не падает ниже марафонского. Может быть ровной "
        "(например, 5 км по дорожке в пороговом темпе) или переменной (чередование быстрых и темповых кусков слитно, "
        "без пауз: 800 м быстро + 1600 м темпово, повторить 4 раза). Если есть смена темпа, но отдыха нет — "
        "это непрерывная переменная, а не интервальная.\n"
        "2. Интервальная — рабочие отрезки 400 м и длиннее (например, 6×800 м), а для медленного бегуна и 300 м, между ними отдых трусцой "
        "или шагом. Отдых оцениваем по времени (относительно времени работы) и по темпу: трусца быстрее easy — "
        "активный отдых, восстановление неполное.\n"
        "3. Скоростная — короткие отрезки до 300 м (например, 8×200 м), отдых длиннее работы в 1.5–2 раза "
        "(200 м шагом или очень медленной трусцой). Цель — техника на высокой скорости, нейромышечная мощность. "
        "300 м — скоростная работа для быстрого и среднего бегуна, для медленного это интервальная работа.\n"
        "4. Комбинированная — сочетание непрерывной части и интервальной/скоростной. Обычно непрерывная "
        "идёт первой (например, 15–20 мин порогового бега), затем интервалы (5×400 м с отдыхом). Второй кусок "
        "оцениваем с поправкой на накопленную усталость: выполнимость ниже, чем у свежего, лимиты объёма "
        "в МПК+ сокращаются.\n"
        "5. Длительный бег — непрерывная работа в зонах easy/marathon, обычно от 40 минут. На стадионе редко, "
        "но бывает: монотонный бег по дорожке. Цель — аэробная база, восстановление.\n\n\n"
        "4. ВЫПОЛНИМОСТЬ\n\n"
        "ВЫПОЛНИМОСТЬ — вероятность дойти до конца в заданных темпах, не развалившись "
        "на последних повторах. Оценивай по темпам и структуре, плавно, не порогами:\n"
        "– СНАЧАЛА определи тип работы по структуре и темпам групп. НЕПРЕРЫВНАЯ работа — между "
        ...
        "в зоне marathon и быстрее (это не отдых, а другая рабочая часть). ИНТЕРВАЛЬНАЯ работа — "
        "между отрезками трусца в зоне easy или медленнее длиной от половины рабочего отрезка, "
        "или шаг/стоя. Промежуточный случай — отдых класса «короткий» (см. СВЯЗЬ ТИПА РАБОТЫ С ЗОНАМИ) "
        "или трусца в зоне между easy и marathon: отдых неполный, работа ближе к непрерывной.\n"
        "– лимиты объёма ниже действуют только для непрерывной работы (и с оговоркой — для "
        "промежуточного случая). В интервальной работе суммарные минуты в зонах для выполнимости "
        "не считай — у медленных групп их больше просто потому, что та же дистанция бежится "
        "дольше; там выполнимость определяется тем, удержит ли атлет темп отрезка на всех "
        ...
        "групп и к 40-50 у медленных;\n"
        "– условия независимы — достаточно одного, чтобы вероятность упала;\n"
        "– правило «зазора от границы зоны»: если заданный темп отстоит от границы целевой зоны менее чем на 10 сек/км — "
        "считай, что работа фактически выполняется в этой зоне (полностью или почти полностью). При зазоре 10–20 сек/км — "
        "считай, что в зону уходит примерно половина времени отрезков. При зазоре более 20 сек/км — можно считать, "
        "что граница не превышается. Это правило применяется для непрерывной работы (в т.ч. переменной без отдыха), "
        "где нет чётких пауз для контроля темпа;\n"
        "– короткие быстрые отрезки с ровным темпом: сравнивай темп группы с ориентиром скорости "
        "атлета (дан ниже) или, если его нет, с зоной repetition. ВАЖНО: и ориентир скорости, и зона repetition — "
        "это не темп одного повтора, а темп, который атлет качественно держит до ~10 повторов на отрезках 300-800 м "
        "и до 20-25 повторов на отрезках до 200 м с полноценным "
        "отдыхом; дальше начинается быстрое западание; при активном отдыхе (быстрее easy) — на 2-4 повтора меньше. "
        "Поэтому темп на уровне потолка/repetition или медленнее при числе повторов до 10 — выполним с высокой "
        "вероятностью, не снижай его «на глаз». Быстрее потолка до 5 сек/км — выполнимо; быстрее на ~10 сек/км — ≈ 50%; быстрее на 20 сек/км и больше — ≈ 0;\n"
        "– число повторов и отдых сдвигают эти границы: мало повторов (≤ 6) и длинный "
        "отдых — снижение начинается позже (шкала сдвигается на 10-15 сек/км быстрее); много повторов (≥ 10), отрезки длиннее 200 м "
        "или короткий/быстрый отдых — раньше;\n"
        "– если у группы прогрессия (диапазон темпа), правило другое: сравнивай с потолком "
        "СЕРЕДИНУ диапазона, а не финишный край. Превышение потолка до 5 сек/км "
        "выполнимо и на всех повторах; чем больше превышение, тем меньше повторов "
        "на нём реально удержать. Выполнимость падает, только когда быстрее потолка "
        "на 10+ сек/км идёт больше трети повторов.\n\n\n"
        "5. СТИМУЛ\n\n"
        "СТИМУЛ — сколько тренировка даёт этому атлету в целевой для неё зоне. Стимул МОНОТОННО РАСТЁТ "
        "от медленных групп к быстрым и НЕ падает у групп быстрее целевой зоны — быстрые группы "
        "ограничивает только выполнимость. Определяется попаданием рабочего темпа группы "
        "в целевую зону типа работы (см. СВЯЗЬ ТИПА РАБОТЫ С ЗОНАМИ ниже), "
        "а не количеством минут — больше минут у медленной группы не значит больше стимула:\n"
        "– высокий, когда темп группы лежит в целевой зоне или быстрее: для коротких ≤300 м — repetition; "
        "для отрезков 400-600 м — interval; для 800-1600 м — между threshold и interval "
        "(для быстрого и среднего бегуна стимул около threshold почти пропадает для такой работы, "
        "а у медленных бегунов снижается выполнимость, если темп близко к interval); "
        "для отрезков длиннее 1600 м и непрерывной работы — threshold "
        "(быстрый и средний бегун может немного быстрее threshold, а медленный немного тише);\n"
        "– падает с каждой зоной ниже целевой: группа на зону ниже — стимул слабый, на две — ≈ 0; "
        "база медленнее marathon-темпа атлета — ≈ 0, даже если пробежать её тривиально;\n"
        "– в непрерывной работе дополнительно: ≈ 0, если весь объём идёт медленнее пороговой зоны;\n"
        "– для скоростной работы (короткие отрезки) смотри на попадание в зону repetition "
        ...
        "ПОДХОДИМОСТЬ: максимальные проценты — там, где оба множителя высоки. Ориентиры "
        "по шкале: 85-100 — точное попадание в целевые зоны, вероятность выполнить "
        "целиком в целевых темпах высокая, срыв маловероятен; "
        "65-85 — хороший вариант с небольшим компромиссом по стимулу или выполнимости, "
        "вероятность выполнить высокая, но с ощутимым риском недовыполнения "
        "на последних повторах; "
        "40-65 — рабочий запасной вариант с ощутимой потерей; 0-25 — отброшенные на шаге 1.\n\n\n"
        "7. СВЯЗЬ ТИПА РАБОТЫ С ЗОНАМИ\n\n"
        "СВЯЗЬ ТИПА РАБОТЫ С ЗОНАМИ — критично для выбора группы:\n"
        "Смотри на СУТЬ тренировки (ниже) и выбирай ориентир по зонам:\n"
        "- Скоростная работа (короткие отрезки ≤300м, отдых длиннее работы в 1.5-2+ раза):\n"
        "  ориентир — зона repetition, а если дан ориентир скорости атлета на эту дистанцию — он.\n"
        "  Сравнивай СЕРЕДИНУ диапазона группы с ориентиром: до 5 сек/км быстрее или до 10 медленнее —\n"
        "  попадание; заметно медленнее — стимул падает; быстрее на 10+ сек/км — выполнимость падает\n"
        "  (см. ВЫПОЛНИМОСТЬ). Целевая скорость = максимум держимый БЕЗ разрушения техники\n"
        "  на ПОСЛЕДНЕМ повторе (~90-95% максимума), первый повтор 'быстро но свободно', не рекордный.\n"
        "  Не занижай до threshold/interval.\n"
        "- МПК-работа (средние отрезки 400-600м, неполный отдых ≤ длины работы; для медленного бегуна сюда же 300 м):\n"
        "  ориентир — зона interval.\n"
        "- Длинные отрезки 800-1600 м с коротким отдыхом: ориентир — между threshold и interval "
        "(у быстрых групп ближе к interval, у медленных — к верхней границе threshold). "
        "При этом учти, что для быстрого и среднего бегуна стимул около threshold почти пропадает для такой работы. "
        "А у медленных бегунов снижается выполнимость, если темп близко к interval.\n"
        "- Темповая/ПАНО (отрезки длиннее 1600 м, непрерывная работа): ориентир — зона threshold. "
        "Быстрые группы могут немного быстрее threshold, а медленные немного тише.\n"
        "- СОСТАВНОЙ отрезок с финишным ускорением (база+ускорение без отдыха внутри) — "
        "ЭТО ИНТЕРВАЛЬНАЯ работа, НЕ темповая: ориентир базы — верх threshold…interval, "
        "ускорения — interval…repetition. Не подбирай группу по близости базы к чистому "
        "threshold — слишком мягкая база (около marathon-темпа) убивает стимул.\n"
        "ОТДЫХ классифицируй по R:W = время отдыха / время работы. Для 200 м: полный ≥1.5, "
        "значительный 1.0–1.5, умеренный 0.6–1.0, короткий <0.6; для 250–300 м: полный 1.2–1.5, "
        "значительный 0.9–1.2, умеренный 0.5–0.9, короткий <0.5.\n"
        "Для 400–800 м: полный 1.0–1.5, значительный 0.75–1.0, умеренный 0.5–0.75, короткий <0.5.\n"
        "Для 1000–1600 м: полный 0.75–1.0, значительный 0.5–0.75, умеренный 0.3–0.5, короткий <0.3.\n"
        "Заполнение только бегом: трусца медленнее easy — класс не меняется; трусца быстрее easy — "
        ...
        "При отдыхе класса умеренный и дольше лимиты объёма непрерывной работы не применяй; "
        "при коротком — применяй с оговоркой.\n"
        "- Длительный бег: зоны marathon/easy.\n"
        ...
        "- Прогрессия идёт ОТ ПОВТОРА К ПОВТОРУ, а НЕ внутри отрезка: внутри одного\n"
        "  повтора темп ровный, а диапазон группы (напр. 4:00→3:45) означает «первые\n"
        "  повторы медленнее, последние быстрее», а не разгон внутри отрезка. Набегание —\n"
        "  СТУПЕНЬ (отрезок бежится ровно, но быстрее базы), если в анонсе не сказано иное\n"
        ...
        "ориентира — в соседнюю зону — допустим, если структура и восстановление "
        "позволяют; недопустима тренировка целиком выше ориентира.\n"
        "Не занижай без причины: недогруз — такая же ошибка,\n"
        "как перегруз; консервативность оправдана большим объёмом\n"
        ...
        "но медленнее 2; 3.5 быстрее 4, но медленнее 3. Таких групп может НЕ быть в этой "
        "тренировке — оценивай и упоминай ТОЛЬКО те группы, что перечислены в разделе ГРУППЫ "
        "ниже. Не добавляй дробные группы сам, если их там нет.\n"
        "Правило эпитетов: у recommended_group эпитет всегда optimal, какой бы ни был её процент. "
        "Группы с номером МЕНЬШЕ рекомендованной — быстрее: «хороший вариант с небольшим компромиссом» "
        "(по шкале ПОДХОДИМОСТИ) — ambitious, «рабочий запасной вариант с ощутимой потерей» — hard, "
        "отброшенная — risk. Группы с номером БОЛЬШЕ — медленнее: то же соответствие — good, easy, tooslow. "
        "Эпитет — одно слово из этого списка, выбирается по ступени подходимости, не по ощущению тяжести.\n"
        "Рассмотри альтернативы: какая группа для скорости, какая для восстановления.\n"
        "Группа с максимальным percentage ДОЛЖНА совпадать с recommended_group.\n"
        "ВАЖНО: верни suitability_percentages для ВСЕХ групп из раздела ГРУППЫ ниже "
        "(только тех, что там реально перечислены), без пропусков и без добавления своих. "
        "НЕ включай группу здоровья в suitability_percentages — "
        "она не оценивается процентом. "
        "Если группа слишком быстрая — ставь risk или hard с низким %, "
        ...
        "В if_feeling_good / if_tired предлагай переход ТОЛЬКО в группы с подходимостью ≥50%. "
        "Если такой альтернативы нет — советуй остаться в рекомендованной группе и скорректировать усилие.\n"
        "Поле warning: проверь recovery_pace рекомендованной группы (видно в GROUPS ниже как 'recovery X:XX'). "
        "Если recovery_pace быстрее 5:30 мин/км — напиши ТОЧНО: "
        "'восстановление между отрезками ~X:XX/км — активное (трусцой)' (вставь реальный темп). "
        "Если recovery_pace >= 5:30 или нет — null. НИКАКОГО другого текста в warning.\n"
        "ВОССТАНОВЛЕНИЕ (TR/Recovery Score) на проценты и recommended_group НЕ влияет — они из оценки "
        ...
        + (
            (
                f"\nОриентир скорости атлета (отрезок {speed_ceilings[0]['distance_m']}м): "
                f"{speed_ceilings[0]['ceiling']}/км — темп, который он нормально держит ~10 повторов подряд, "
                "НЕ максимум одного повтора; более короткие серии может выполнять на 10-15 сек/км быстрее.\n"
            ) if speed_ceilings and len(speed_ceilings) == 1 else (
                "\nОриентир скорости атлета по блокам (темп ~10 повторов подряд, не максимум): " +
                ", ".join(f"{sc['distance_m']}м → {sc['ceiling']}/км" for sc in speed_ceilings) +
                ".\n"
            ) if speed_ceilings else ""
        )
...
def ask_groq(prompt: str, mode: str = "smart") -> dict | None:
    ...
    if mode == "deep":
        model, max_tok, api_timeout = MODEL_DEEP, 64000, 900
    elif mode == "smart":
        model, max_tok, api_timeout = MODEL_SMART, 64000, 600
        # 03.09: 64000 — потолок DeepSeek; при 32000 умный упирался в кэп (ровно 32000 → обрезанный JSON, карточка с нулями)
    else:
...
def format_evening_message(advice: dict, workout: dict, stats: dict | None = None, weather_line: str = "", profile_only: bool = False, has_tracker: bool = True) -> str:
    ...
    work_text = _html.escape(strip_links(workout.get("work_text")))
    if work_text:
...
def build_group_zone_table(analysis: dict, zones_map: dict | None) -> str:
    ...
    for g in (analysis.get("groups") or []):
        ...
        for st in structure:
            ...
            # Разворачиваем повторы в реальную последовательность шагов — непрерывность
            # считается по ней (отдых трусцой между повторами рвёт кусок)
            first_rep = len(steps)
            unrolled = steps * max(reps, 1)
            for idx, (_dist, _pace, _is_work) in enumerate(unrolled):
                if not _pace or _dist <= 0:
                    if _dist > 0 and not _is_work:
                        cont_best = max(cont_best, cont_sec)
                        cont_sec = 0.0
                        itv_cont_best = max(itv_cont_best, itv_cont_sec)
                        itv_cont_sec = 0.0
                    continue
                try:
        ...
        lines.append(
            ...
            f" · МПК+ {itv_sec_total/60:.0f} мин (непрерывно {itv_cont_best/60:.0f})"
        )
    if not lines:
        return ""
    return ("\nСВОДКА ПО ГРУППАМ — ВСЕ ЦИФРЫ УЖЕ ПОСЧИТАНЫ, ПЕРЕСЧИТЫВАТЬ НЕ НУЖНО. "
            "Используй эти данные напрямую для оценки групп без повторного расчёта зон. "
            "Зона и отклонение от ПАНО каждого отрезка, объёмы ПАНО+ и МПК+ тоже даны здесь — "
            "не сопоставляй темпы с зонами и не суммируй время повторно.\n"
            "Легенда: «ПАНО+» = время в темпе не медленнее чем ПАНО плюс 15 сек/км "
            "(то есть пороговая зона и всё, что быстрее); «МПК+» = время в темпе не медленнее "
            "чем МПК плюс 10 сек/км. В скобках после темпа отрезка: зона и отклонение от ПАНО "
            "в сек/км, минус = быстрее порога, плюс = медленнее. Финальный отрезок без заданного "
            "темпа в объёмы не входит и на выбор группы не влияет.\n"
            + "\n".join(lines) + "\n")
...
def build_long_run_prompt(workout: dict, fitness: dict, recovery: dict | None = None, weather_prompt: str = "", recovery_scenario_text: str = "", zones_map: dict | None = None) -> str:
    ...
    parts += [
        ...
        *(["ПОГОДА НА ТРЕНИРОВКУ:", weather_prompt,
        ...
        "В if_feeling_good / if_tired предлагай переход ТОЛЬКО в группы с подходимостью ≥50%. "
        "Если такой альтернативы нет — советуй остаться в рекомендованной группе "
        "и скорректировать усилие (ровный темп / без прогрессии).",
        "",
