    ...
    return (
        ...
        + (
            (
                ...
                "ЕСЛИ В ЗАДАНИИ НЕТ ПРОГРЕССИИ (у группы единый темп, без диапазона) — "
                "проверь только одно: темп группы не быстрее этого потолка. Всё, что ниже про "
                "допуски и финальный край, ПРОПУСТИ и не рассчитывай.\n"
                "ЕСЛИ ПРОГРЕССИЯ ЕСТЬ — сравнивай с потолком СЕРЕДИНУ диапазона. Допустимое "
                "превышение середины над потолком зависит от числа повторов, длины отрезка и отдыха:\n"
                "- 100-200м, повторов ≤6, отдых ≥2× времени работы: до +3 сек/км — допустимо с пометкой ambitious;\n"
                "- 100-200м, повторов 7-8, отдых ≥1.5×: до +2 сек/км — допустимо, но % не выше 55-60;\n"
                "- 100-200м, повторов 9-12: превышение недопустимо — середина не быстрее потолка;\n"
                "- длиннее 200м ИЛИ повторов >12: середина должна быть МЕДЛЕННЕЕ потолка на 2-3 сек/км.\n"
                "Превышение больше допуска — группа risk (% ≤ 10).\n"
                "Финальный край прогрессии сам по себе не блокирует группу, если середина прошла, НО: "
                "финальный темп не быстрее потолка чем на 5 сек/км, И быстрее потолка — не более 1/3 повторов; "
                "нарушены ОБА условия — группа risk даже при допустимой середине.\n"
                ...
                ". Для КАЖДОГО блока сравнивай середину его диапазона с его потолком; допуски"
                " по числу повторов блока: 100-200м и ≤6 повторов (отдых ≥2×) — до +3 сек/км"
                " (ambitious); 7-8 повторов — до +2 (% ≤ 55-60); 9-12 — без превышения;"
                " длиннее 200м или >12 повторов — середина МЕДЛЕННЕЕ потолка на 2-3 сек/км."
                " Группа risk/hard (% ≤ 10), если хоть один блок превышает СВОЙ допуск.\n"
            ) if speed_ceilings else ""
        )
        + "\nПОДХОДИМОСТЬ = СТИМУЛ × ВЕРОЯТНОСТЬ ВЫПОЛНИТЬ задание целиком в заданных темпах; "
        "без финиша стимула нет. ПОРЯДОК: сначала БЕЗ учёта восстановления оцени по цифрам "
        "сводки вероятность выполнить и стимул каждой группы — плавно, не порогами:\n"
        "– объём МПК+: до 6-8 мин не снижает, каждая минута сверх снижает, к 12-14 мин "
        "вероятность ≈ 0 (в переменной работе без отдыха — сверх 5-6, ≈ 0 к 8-10);\n"
        "– непрерывный ПАНО+: сверх ~35-40 мин каждая минута снижает, ≈ 0 к 50-60 "
        "у быстрых групп и к 40-50 у медленных;\n"
        "– стимул ≈ 0, если МПК+ = 0 и ПАНО+ < ~20 мин (для работы на МПК/ПАНО); "
        "группа, чья база медленнее marathon-темпа атлета, стимула почти не даёт "
        "(easy/tooslow), даже если пробежать её тривиально.\n"
        "Невыполнимое и бесполезное отбрасывай сразу — остаются 2-3 реальные группы; "
        "отброшенная группа не может быть optimal, как бы точно ни попадала в зону "
        "(hard/risk или easy/tooslow). Максимальные проценты — там, где оба множителя "
        "высоки. Восстановление учитывай только после этого отсева.\n"
        + "\nОБЪЁМ КАЧЕСТВЕННОЙ РАБОТЫ: цифры ПАНО+ и МПК+ бери из СВОДКИ ПО ГРУППАМ, "
        "сам не считай; как они влияют на выполнимость и стимул — в правиле ПОДХОДИМОСТИ. "
        "Если «восстановительный» отрезок "
        "группы по сводке лежит в зоне threshold или быстрее — серия для этого атлета "
        "превращается в непрерывную переменную работу (быстро + темпово): физиология "
        "пороговая/темповая, не интервальная — учти при выборе группы.\n"
...
def build_group_zone_table(analysis: dict, zones_map: dict | None) -> str:
    ...
    return ("\nСВОДКА ПО ГРУППАМ — ВСЕ ЦИФРЫ УЖЕ ПОСЧИТАНЫ, ПЕРЕСЧИТЫВАТЬ НЕ НУЖНО. "
            ...
            "(«ПАНО+» = время в темпе ПАНО+15 сек/км и быстрее, "
            "«МПК+» = МПК+10 сек/км и быстрее):\n"
            + "\n".join(lines) + "\n")
