...
def build_ai_b_prompt_reco_champion(analysis: dict, user_data: dict, zones_map: dict, recovery: dict | None, recovery_scenario_text: str = "", speed_ceilings: list | None = None) -> str:
    ...
    return (
        ...
        "– объём МПК+: считается СУММАРНОЕ время за тренировку — усталость копится; то, что "
        ...
        "СТИМУЛ — сколько тренировка даёт этому атлету в целевых для неё зонах. Для работы "
        "на МПК/ПАНО растёт с объёмом в этих зонах:\n"
        "– ≈ 0, если МПК+ = 0 и ПАНО+ < ~20 мин;\n"
        "– ≈ 0 у группы, чья база медленнее marathon-темпа атлета, даже если пробежать "
        "её тривиально;\n"
        "– максимален там, где объём в целевых зонах наибольший из тех, что ещё выполнимы;\n"
