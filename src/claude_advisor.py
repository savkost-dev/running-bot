...
def build_ai_b_prompt_reco_challenger(analysis: dict, user_data: dict, zones_map: dict, recovery: dict | None, recovery_scenario_text: str = "", speed_ceilings: list | None = None) -> str:
    ...
    return (
        ...
        "или шагом. Отдых оцениваем по длине (относительно отрезка) и по темпу: трусца быстрее easy — "
        ...
        "в зоне marathon и быстрее (это не отдых, а другая рабочая часть). ИНТЕРВАЛЬНАЯ работа — "
        "между отрезками трусца в зоне easy или медленнее длиной от половины рабочего отрезка, "
        "или шаг/стоя. Промежуточный случай — трусца короче четверти отрезка или в зоне между easy "
        "и marathon: отдых неполный, работа ближе к непрерывной.\n"
        "– лимиты объёма ниже действуют только для непрерывной работы (и с оговоркой — для "
        "промежуточного случая). В интервальной работе суммарные минуты в зонах для выполнимости "
        "не считай — у медленных групп их больше просто потому, что та же дистанция бежится "
        ...
        "ускорения — interval…repetition. Не подбирай группу по близости базы к чистому "
        "threshold — слишком мягкая база (около marathon-темпа) убивает стимул.\n"
        "ОТДЫХ классифицируй ОТНОШЕНИЕМ к работе, а не абсолютом: ≥1/2 длины отрезка — "
        "значительный (можно держать верх зоны-ориентира); от 1/4 до 1/2 — умеренный "
        "(середина ориентира; лимиты объёма непрерывной работы не применяй); ≤1/4 — "
        "короткий (низ ориентира, работа ближе к непрерывной — лимиты применяй с оговоркой). "
        "Не называй отдых коротким по умолчанию.\n"
        "Вторая ось — ТЕМП отдыха: если recovery-темп группы быстрее easy-зоны атлета — "
        "это АКТИВНЫЙ отдых (бег, а не трусца): восстановление неполное даже при большой "
        "длине — сдвигайся к нижней части ориентира.\n"
        "- Длительный бег: зоны marathon/easy.\n"
