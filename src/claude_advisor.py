...
def _build_analyze_prompt(raw_text: str, comments_text: str) -> str:
    ...
    return (
        ...
        f"{raw_text}\n\n"
        "КОММЕНТАРИИ К ПОСТУ:\n"
        "Строки с пометкой [ИНСТРУКЦИЯ АДМИНИСТРАТОРА] — уточнение задания с ВЫСШИМ "
        "приоритетом: важнее основного текста анонса и любых других комментариев. "
        "Из остальных комментариев бери ТОЛЬКО факты: дополнительные группы с темпами, "
        "размер круга дорожки, поправки дистанций. Мнения, домыслы и интерпретации "
        "участников (что считать отдыхом, какая физиология у отрезка, насколько тяжело) "
        "ИГНОРИРУЙ — структуру и характер работы определяй по анонсу. "
        "Бег в темпе около ПАНО и быстрее НЕ называй восстановлением или отдыхом "
        "ни при каких комментариях: это смена темпа, темповый или пороговый отрезок; "
        "восстановление — только бег существенно медленнее ПАНО.\n"
        f"{comments_text}\n\n"
        "ЗАДАЧА 1 — Валидация:\n"
