...
def build_ai_b_prompt_reco_champion(analysis: dict, user_data: dict, zones_map: dict, recovery: dict | None, recovery_scenario_text: str = "", speed_ceilings: list | None = None) -> str:
    ...
    return (
        ...
        "сам не считай. Если непрерывный кусок ПАНО+ у группы больше ~20-25 минут — "
        "для этого атлета тренировка «тяжёлая» или «очень тяжёлая», даже если "
        "в описании есть восстановительные отрезки. Если «восстановительный» отрезок "
        ...
        "Вероятность выполнить задание оценивай по правилу ПОДХОДИМОСТИ ниже.\n"
        ...
        "без финиша стимула нет. Стимул растёт от медленных групп к быстрым: группа, чья база "
        "медленнее marathon-темпа атлета, почти не даёт стимула — низкий процент "
        "(easy/tooslow), даже если пробежать её тривиально. Вероятность выполнить падает "
        "от медленных групп к быстрым — оценивай по цифрам сводки: с приближением "
        "суммарного МПК+ к 10-12 минутам она резко падает, а в переменной работе "
        "без отдыха — уже при 8-10 минутах непрерывного МПК+; условия независимы — "
        "достаточно любого одного, суммарное считается всегда; непрерывная работа "
        "в ПАНО+ терпима до 50-60 минут у сильных групп и до 40-50 у слабых, дальше "
        "риск не закончить резко растёт; низкое восстановление и превышение потолка "
        "темпа тоже её снижают. Группа, которую атлет скорее всего не закончит в заданных "
        "темпах, не может быть optimal, как бы точно ни попадала в зону — hard/risk. "
        "Максимальные проценты — там, где оба множителя высоки.\n"
