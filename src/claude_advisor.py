...
def _build_analyze_prompt(raw_text: str, comments_text: str) -> str:
    ...
    return (
        ...
        "  работы или >60 сек трусцы): лактат выводится, КрФ восстанавливается —\n"
        "  СКОРОСТЬ / нейромышечная мощность / техника на скорости, НЕ МПК/ПАНО.\n"
        "  Каждый отрезок бежится свежим на 90-95% максимума.\n"
...
def build_ai_b_prompt(analysis: dict, user_data: dict, zones_map: dict, recovery: dict | None, recovery_scenario_text: str = "") -> str:
    ...
    return (
        ...
        "- Скоростная/нейромышечная (короткие отрезки ≤200м, отдых длиннее работы в 1.5-2+ раза,\n"
        "  КрФ восстанавливается, НЕ МПК/ПАНО): ориентир — зона repetition и БЫСТРЕЕ.\n"
        "  Полный отдых позволяет держать высокий темп без закисления. НЕ занижай до\n"
        "  threshold/interval — это сделало бы работу слишком медленной для её цели.\n"
        ...
        "без пропусков. НЕ включай группу здоровья в suitability_percentages — "
