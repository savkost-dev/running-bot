...
def build_ai_b_prompt_reco_champion(analysis: dict, user_data: dict, zones_map: dict, recovery: dict | None, recovery_scenario_text: str = "", speed_ceilings: list | None = None) -> str:
    ...
    return (
        ...
        "Предел МПК: если суммарное МПК+ группы больше 10-12 минут — группа risk "
        "с процентом ≤15 независимо от попадания в целевую зону; в непрерывной "
        "переменной работе (без восстановительных пауз) предел жёстче — 8-10 минут "
        "непрерывного МПК+ (цифра «непрерывно» у МПК+ в сводке).\n"
