...
def build_ai_b_prompt_reco_challenger(analysis: dict, user_data: dict, zones_map: dict, recovery: dict | None, recovery_scenario_text: str = "", speed_ceilings: list | None = None) -> str:
    ...
    return (
        ...
        "2. Интервальная — рабочие отрезки 300 м и длиннее (например, 6×800 м), между ними отдых трусцой "
        ...
        "3. Скоростная — короткие отрезки до 200 м (например, 8×200 м), отдых длиннее работы в 1.5–2 раза "
        ...
        "в целевую зону типа работы (см. СВЯЗЬ ТИПА РАБОТЫ С ЗОНАМИ ниже), "
        "а не количеством минут — больше минут у медленной группы не значит больше стимула:\n"
        "– высокий, когда темп группы лежит в целевой зоне или быстрее: для коротких ≤200 м — repetition; "
        "для отрезков 300-600 м — interval; для 800-1600 м — между threshold и interval "
