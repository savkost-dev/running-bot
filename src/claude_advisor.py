...
def build_ai_b_prompt_reco_challenger(analysis: dict, user_data: dict, zones_map: dict, recovery: dict | None, recovery_scenario_text: str = "", speed_ceilings: list | None = None) -> str:
    ...
    return (
        ...
        "групп и к 40-50 у медленных;\n"
        "– условия независимы — достаточно одного, чтобы вероятность упала;\n"
        "– правило «зазора от границы зоны»: если заданный темп отстоит от границы целевой зоны менее чем на 5 сек/км — "
        "считай, что работа фактически выполняется в этой зоне (полностью или почти полностью). При зазоре 5–10 сек/км — "
        "считай, что в зону уходит примерно половина времени отрезков. При зазоре более 10 сек/км — можно считать, "
